//! `[UnorientedDigraph]`s and their flag implementation.
...
impl UnorientedDigraph {
    ...
    /// ```
    /// use flag_algebra::flags::UnorientedDigraph;
    ///
    /// // Oriented path with 3 vertices `0 -> 1 -> 2`
    /// let p3 = UnorientedDigraph::new(3, [(0, 1), (1, 2)]);
    /// ```
    ...
    /// ```
    /// use flag_algebra::flags::UnorientedDigraph;
    /// assert_eq!(UnorientedDigraph::empty(4), UnorientedDigraph::new(4, []));
    /// ```
    ...
    /// ```
    /// use flag_algebra::flags::{UnorientedDigraph, Arc};
    /// let p3 = UnorientedDigraph::new(3, [(0,1), (1,2)]);
...
impl fmt::Display for UnorientedDigraph {
...
fn check_arc((u, v): (usize, usize), graph_size: usize) {
    ...
    assert!(
        u != v,
        "Invalid arc ({u}, {v}): UnorientedDigraph have no loop"
    );
}

impl Canonize for UnorientedDigraph {
    fn size(&self) -> usize {
        self.size
    }
...
impl Flag for UnorientedDigraph {
    ...
    const NAME: &'static str = "UnorientedDigraph";
...
/// Indicator for UnorientedDigraph without directed triangle.
///
/// Makes `SubClass<UnorientedDigraph, TriangleFree>` usable as flags.
#[derive(Debug, Clone, Copy)]
pub enum TriangleFree {}

impl SubFlag<UnorientedDigraph> for TriangleFree {
    const SUBCLASS_NAME: &'static str = "Triangle-free UnorientedDigraph";

    fn is_in_subclass(flag: &UnorientedDigraph) -> bool {
        flag.is_triangle_free()
    }
}

impl<A> SubClass<UnorientedDigraph, A> {
    pub fn size(&self) -> usize {
        self.content.size()
    }
}
